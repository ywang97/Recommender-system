...
def sgd_step(data, x, lam, step):
    ...
    dJdu = -(r-v[i].T@u[a]-b_u[a]-b_v[i])*v[i]+lam_u[a]*u[a]
    dJdv = -(r-v[i].T@u[a]-b_u[a]-b_v[i])*u[a]+lam_v[i]*v[i]
    dJdbu = -(r-v[i].T@u[a]-b_u[a]-b_v[i])
    dJdbv = -(r-v[i].T@u[a]-b_u[a]-b_v[i])
    u[a] = u[a] - dJdu*step
    v[i] = v[i] - dJdv*step
    b_u[a] = b_u[a] - dJdbu*step
    b_v[i] = b_v[i] - dJdbv*step
    return x
...
##cosines = dict() #cosine similarity
##for i in movies_dict:
##    dot = np.inner(v[i].T,v[2628].T)
##    mags = np.linalg.norm(v[i])*np.linalg.norm(v[2628])
##    div = abs(dot/mags)
##    cosines[i] = div
##sims = []
##for i in sorted(cosines, key = lambda k: cosines[k], reverse=True)[:11]:
##    sims.append(i)
##print(sims)
...
##        if i!=j:
##            div = v[i].T.dot(v[j])/(np.linalg.norm(v[i])*np.linalg.norm(v[j]))
##            total= total + div
##            count+=1
##ave = total/count
##print(ave)

rev_genre = dict((g, []) for g in genres)
for idd in genres_dict:
    for gen in genres_dict[idd]:
        rev_genre[gen].append(idd)

##ll = []
##for gen in rev_genre:
##    total, count = 0, 0
##    for i in rev_genre[gen]:
##        for j in rev_genre[gen]:
##            if i==j: continue
##            div = v[i].T.dot(v[j])/(np.linalg.norm(v[i])*np.linalg.norm(v[j]))
##            total= total + div
##            count+=1
##    ave = total/count
##    ll.append((gen, ave))
##mmax = max(ll, key = lambda x: x[1])
##print(mmax)

comedy = rev_genre['Comedy']
new = dict()
for gen in rev_genre:
    sim, count = 0, 0
    if gen == 'Comedy': continue
    for i in rev_genre[gen]:
        for j in comedy:
            if i == j: continue
            temp = v[i].T.dot(v[j])/(np.linalg.norm(v[i])*np.linalg.norm(v[j]))
            sim += temp
            count += 1
    if count!=0:
        ave = sim / count
...
mmin = min(new, key = lambda x: new[x])
mmax = max(new, key = lambda x: new[x])
